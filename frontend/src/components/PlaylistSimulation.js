        ...
        try {
            ...
            for (const screen of allScreens) {
                try {
                    ...
                    // Check if any zone has one of the selected playlists
                    // Playlists can be stored in either playlist_id OR content_id (when content_type='playlist')
                    const hasPlaylist = zonesRes.data.some(zone => {
                        // Check direct playlist_id field
                        if (zone.playlist_id && playlistIds.includes(zone.playlist_id)) {
                            return true;
                        }
                        // Check content_id when content_type is 'playlist'
                        if (zone.content_type === 'playlist' && zone.content_id && playlistIds.includes(zone.content_id)) {
                            return true;
                        }
                        return false;
                    });
        ...
        <div className="relative w-full h-full bg-slate-900 flex flex-col items-center justify-center p-8">
            ...
            <div className="mb-6 text-center">
                ...
                <p className="text-white/60 text-sm">
                    {screens.length} ecran{screens.length !== 1 ? 'e' : ''}
                </p>
                {screens.length > 0 && screens.length < 3 && (
                    <div className="mt-3 inline-flex items-center gap-2 bg-amber-500/20 border border-amber-500/30 text-amber-200 px-4 py-2 rounded-lg text-xs">
                        ...
                        <span>Pentru acuratețe maximă, selectează playlist-uri care au 3 ecrane atribuite</span>
                    </div>
                )}
            </div>
            ...
            {screens.length === 0 ? (
                ...
                >
                    ...
                    >
                        {screens.slice(0, 3).map((screen, idx) => (
                            <div
                                key={screen.id}
